    ...
    def observation_space(self):
        ...
        if self.obs_space_type == "box":
            return gym.spaces.Dict(
                {
                    "achieved_goal": gym.spaces.Box(low=-200.0, high=200.0, shape=(3,), dtype=np.float32),
                    "desired_goal": gym.spaces.Box(low=-200.0, high=200.0, shape=(3,), dtype=np.float32),
                    "observation": gym.spaces.Box(low=-200.0, high=200.0, shape=(25,), dtype=np.float32),
                }
            )
        elif self.obs_space_type == "image":
            return gym.spaces.Dict(
                {
                    "dummy": gym.spaces.Box(low=0, high=255, shape=(100, 100, 3), dtype=np.uint8),
                }
            )
        else:
