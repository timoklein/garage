...
def test_output_values(output_dim, kernel_sizes, hidden_channels, strides, paddings):
    input_width = 32
    input_height = 32
    in_channel = 3
    input_shape = (in_channel, input_height, input_width)
    spec = InOutSpec(
        akro.Box(shape=input_shape, low=-np.inf, high=np.inf), akro.Box(shape=(output_dim,), low=-np.inf, high=np.inf)
    )
    obs = torch.rand(input_shape)

    module = DiscreteCNNModule(
        spec=spec,
        image_format="NCHW",
    ...
    cnn = CNNModule(
        spec=InOutSpec(akro.Box(shape=input_shape, low=-np.inf, high=np.inf), None),
        image_format="NCHW",
        hidden_channels=hidden_channels,
        kernel_sizes=kernel_sizes,
        strides=strides,
...
@pytest.mark.parametrize("output_dim, hidden_channels, kernel_sizes, strides", [(1, (32,), (1,), (1,))])
def test_without_nonlinearity(output_dim, hidden_channels, kernel_sizes, strides):
    input_width = 32
    input_height = 32
    in_channel = 3
    input_shape = (in_channel, input_height, input_width)
