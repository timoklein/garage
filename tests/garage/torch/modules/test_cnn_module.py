...
class TestCNNModule:
    ...
    def setup_method(self):
        ...
        self.input_spec = InOutSpec(
            akro.Box(shape=[self.in_channel, self.input_height, self.input_width], high=np.inf, low=np.inf), None
        )
        self.input = torch.zeros(
            (self.batch_size, self.in_channel, self.input_height, self.input_width), dtype=self.dtype
    ...
    def test_output_values(self, kernel_sizes, hidden_channels, strides, paddings):
        ...
        output2 = module_with_nonlinear_module_instance_and_function(self.input)

        current_size = self.input_width
        for filter_size, stride, padding in zip(kernel_sizes, strides, paddings):
            # padding = float((filter_size - 1) / 2) # P = (F - 1) /2
            current_size = (
                int((current_size - filter_size + padding * 2) / stride) + 1
            )  # conv formula = ((W - F + 2P) / S) + 1
        flatten_shape = current_size * current_size * hidden_channels[-1]

        expected_output = torch.zeros((self.batch_size, flatten_shape))

        assert np.array_equal(torch.all(torch.eq(output1, expected_output)), True)
    ...
    def test_output_values_with_unequal_stride_with_padding(self, hidden_channels, kernel_sizes, strides, paddings):
        ...
        expected_output = torch.zeros((self.batch_size, flatten_shape))
        assert np.array_equal(torch.all(torch.eq(output, expected_output)), True)
    ...
    def test_is_pickleable(self, hidden_channels, kernel_sizes, strides):
        ...
        """
        model = CNNModule(
            self.input_spec, image_format="NCHW", hidden_channels=hidden_channels, kernel_sizes=kernel_sizes, strides=strides
        )
    ...
    @pytest.mark.parametrize(
        "kernel_sizes, hidden_channels, " "strides, pool_shape, pool_stride",
        [
            ((1,), (32,), (1,), 1, 1),
            ((3,), (32,), (1,), 1, 1),
            ((3,), (32,), (2,), 2, 2),
    ...
    def test_output_with_max_pooling(self, kernel_sizes, hidden_channels, strides, pool_shape, pool_stride):
        model = CNNModule(
            self.input_spec,
            image_format="NCHW",
            hidden_channels=hidden_channels,
...
def test_set_output_size(kernel_sizes, hidden_channels, strides, pool_shape, pool_stride):
    spec = InOutSpec(akro.Box(shape=[3, 19, 15], high=np.inf, low=-np.inf), akro.Box(shape=[200], high=np.inf, low=-np.inf))
    model = CNNModule(
        spec,
        image_format="NCHW",
        hidden_channels=hidden_channels,
...
def test_check_spec():
    ...
    with pytest.raises(ValueError, match="should have three dimensions"):
        ...
        _check_spec(InOutSpec(akro.Box(shape=[1, 1, 1, 1], low=-np.inf, high=np.inf), None), "NCHW")
    with pytest.raises(ValueError, match="akro.Box with a single dimension"):
        # Output is not one-dimensional
        _check_spec(
            InOutSpec(
                akro.Box(shape=[1, 1, 1], low=-np.inf, high=np.inf),
                akro.Box(
                    shape=[1, 1],
                    low=-np.inf,
                    high=np.inf,
    ...
    with pytest.warns(UserWarning):
        ...
        _check_spec(InOutSpec(akro.Box(shape=[4, 1, 1], low=-np.inf, high=np.inf), None), "NCHW")
