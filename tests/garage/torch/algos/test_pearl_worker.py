...
def test_methods():
    ...
    policy = TanhGaussianMLPPolicy(
        env_spec=augmented_env_spec, hidden_sizes=(3, 5, 7), hidden_nonlinearity=F.relu, output_nonlinearity=None
    )
    ...
    worker1 = PEARLWorker(seed=1, max_episode_length=max_episode_length, worker_number=1)
    ...
    worker2 = PEARLWorker(
        seed=1, max_episode_length=max_episode_length, worker_number=1, deterministic=True, accum_context=True
    )
    worker2.update_agent(context_policy)
    worker2.update_env(env_spec)
    episodes = worker2.rollout()

    assert context_policy.context.shape == (1, max_episode_length, encoder_input_dim)
    assert episodes.observations.shape == (max_episode_length, obs_dim)
    assert episodes.actions.shape == (max_episode_length, action_dim)
    assert episodes.rewards.shape == (max_episode_length,)
