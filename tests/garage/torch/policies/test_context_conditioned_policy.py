...
class TestContextConditionedPolicy:
    ...
    def setup_method(self):
        ...
        latent_space = akro.Box(low=-1, high=1, shape=(self.latent_dim,), dtype=np.float32)

        # add latent space to observation space to create a new space
        augmented_obs_space = akro.Tuple((self.env_spec.observation_space, latent_space))
    ...
    def test_update_context(self):
        ...
        assert torch.all(torch.eq(self.module.context, torch.ones(updates, self.encoder_input_dim)))
