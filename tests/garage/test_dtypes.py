...
def test_new_eps(eps_data):
    ...
    assert t.episode_infos["task_one_hot"][0].shape == eps_data["episode_infos"]["task_one_hot"][0].shape
...
def test_last_obs_env_spec_mismatch_eps(eps_data):
    with pytest.raises(ValueError, match=("last_observations must have the " "same number of entries")):
...
def test_act_box_env_spec_mismatch_eps(eps_data):
    with pytest.raises(ValueError, match="Each action has shape"):
        eps_data["env_spec"] = EnvSpec(
            eps_data["env_spec"].observation_space, akro.Box(low=1, high=np.inf, shape=(4, 3, 2), dtype=np.float32)
        )
        t = EpisodeBatch(**eps_data)
        del t
...
def test_env_infos_batch_mismatch_eps(eps_data):
    with pytest.raises(ValueError, match="Entry 'goal' in env_infos has batch size 141"):
...
def test_agent_infos_batch_mismatch_eps(eps_data):
    with pytest.raises(ValueError, match="Entry 'hidden' in agent_infos has batch size 141"):
        eps_data["agent_infos"]["hidden"] = eps_data["agent_infos"]["hidden"][:-1]
        t = EpisodeBatch(**eps_data)
        del t
...
def test_episode_infos_batch_mismatch_eps(eps_data):
    with pytest.raises(ValueError, match="Entry 'task_one_hot' in episode_infos"):
        eps_data["episode_infos"]["task_one_hot"] = eps_data["episode_infos"]["task_one_hot"][:-1]
        t = EpisodeBatch(**eps_data)
        del t
...
def test_to_epsbatch_list(eps_data):
    ...
    start = 0
    for length, last_obs, s in zip(eps_data["lengths"], eps_data["last_observations"], t_list):
        stop = start + length
        assert (s["observations"] == eps_data["observations"][start:stop]).all()
        assert (s["next_observations"] == np.concatenate((eps_data["observations"][start + 1 : stop], [last_obs]))).all()
...
def test_episodes_padding_tensors(eps_data):
    ...
    for key in agent_infos.keys():
        assert agent_infos[key].shape == (N, max_ep_l, *t.agent_infos[key][0].shape)
    ...
    for i, length in enumerate(t.lengths):
        ...
        for key in agent_infos.keys():
            assert (agent_infos[key][i][:length] == t.agent_infos[key][start:stop]).all()
...
def test_get_step_type():
    step_type = StepType.get_step_type(step_cnt=1, max_episode_length=5, done=False)
    assert step_type == StepType.FIRST
    step_type = StepType.get_step_type(step_cnt=2, max_episode_length=5, done=False)
    assert step_type == StepType.MID
    step_type = StepType.get_step_type(step_cnt=2, max_episode_length=None, done=False)
    assert step_type == StepType.MID
    step_type = StepType.get_step_type(step_cnt=5, max_episode_length=5, done=False)
    assert step_type == StepType.TIMEOUT
    step_type = StepType.get_step_type(step_cnt=5, max_episode_length=5, done=True)
    assert step_type == StepType.TIMEOUT
    step_type = StepType.get_step_type(step_cnt=1, max_episode_length=5, done=True)
    assert step_type == StepType.TERMINAL
    with pytest.raises(ValueError):
        step_type = StepType.get_step_type(step_cnt=0, max_episode_length=5, done=False)
...
def test_from_env_step_time_step(sample_data):
    ...
    time_step_new = TimeStep.from_env_step(
        env_step=env_step, last_observation=last_observation, agent_info=agent_info, episode_info=episode_info
    )
...
def test_act_box_env_spec_mismatch_batch(batch_data):
    with pytest.raises(ValueError, match="Each action has"):
        batch_data["env_spec"] = EnvSpec(
            batch_data["env_spec"].observation_space, akro.Box(low=1, high=np.inf, shape=(4, 3, 2), dtype=np.float32)
        )
        s = TimeStepBatch(**batch_data)
        del s
...
def test_env_infos_batch_mismatch_batch(batch_data):
    with pytest.raises(ValueError, match="Entry 'goal' in env_infos has batch size 1"):
...
def test_agent_infos_batch_mismatch_batch(batch_data):
    with pytest.raises(ValueError, match="Entry 'hidden' in agent_infos has batch size 1"):
...
def test_concatenate_batch(batch_data):
    ...
    new_next_obs = np.concatenate([batch_data["next_observations"], batch_data["next_observations"]])
    ...
    new_step_types = np.concatenate([batch_data["step_types"], batch_data["step_types"]])
    new_env_infos = {k: np.concatenate([b.env_infos[k] for b in batches]) for k in batches[0].env_infos.keys()}
    new_agent_infos = {k: np.concatenate([b.agent_infos[k] for b in batches]) for k in batches[0].agent_infos.keys()}
...
def test_split_batch(batch_data):
    ...
    for i, batch in enumerate(batches):
        ...
        assert np.array_equal(batch.next_observations, [batch_data["next_observations"][i]])
        ...
        for key in batch.env_infos:
            ...
            assert np.array_equal(batch.env_infos[key], [batch_data["env_infos"][key][i]])
        for key in batch.agent_infos:
            assert key in batch_data["agent_infos"]
            assert np.array_equal(batch.agent_infos[key], [batch_data["agent_infos"][key][i]])
        for key in batch.episode_infos:
            assert key in batch_data["episode_infos"]
            assert np.array_equal(batch.episode_infos[key], [batch_data["episode_infos"][key][i]])
...
def test_episode_info_wrong_length(eps_data):
    with pytest.raises(ValueError, match="Entry 'task_one_hot' in episode_infos has"):
