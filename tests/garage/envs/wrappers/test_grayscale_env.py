...
class TestGrayscale:
    ...
    def test_grayscale_observation_space(self):
        assert self.env_g.observation_space.shape == (self.env.observation_space.shape[:-1])
    ...
    def test_grayscale_reset(self):
        ...
        """
        grayscale_output = np.round(np.dot(self.env.reset()[:, :, :3], [0.2125, 0.7154, 0.0721])).astype(np.uint8)
    ...
    def test_grayscale_step(self):
        ...
        grayscale_output = np.round(np.dot(obs[:, :, :3], [0.2125, 0.7154, 0.0721])).astype(np.uint8)
