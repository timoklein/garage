...
from garage.envs.multi_env_wrapper import MultiEnvWrapper, round_robin_strategy, uniform_random_strategy


class TestMultiEnvWrapper:
    """Tests for garage.envs.multi_env_wrapper"""

    def _init_multi_env_wrapper(self, env_names, sample_strategy=uniform_random_strategy):
    ...
    def test_default_active_task_is_none(self):
        ...
        envs = ["CartPole-v0", "CartPole-v1"]
        mt_env = self._init_multi_env_wrapper(envs, sample_strategy=round_robin_strategy)
    ...
    def test_one_hot_observation_space(self):
        ...
        obs_space = akro.Box(np.concatenate([cartpole_lb, np.zeros(2)]), np.concatenate([cartpole_ub, np.ones(2)]))
    ...
    def test_uniform_random_sample_strategy(self):
        ...
        mt_env = self._init_multi_env_wrapper(envs, sample_strategy=uniform_random_strategy)
        tasks = []
        for _ in envs:
            mt_env.reset()
            es = mt_env.step(1)
