...
def eps_eq(true_eps, test_eps):
    return np.allclose(true_eps.observations, test_eps.observations) and np.allclose(true_eps.actions, test_eps.actions)
...
def test_rollout(env, policy, timesteps_per_call):
    worker = FragmentWorker(
        seed=SEED, max_episode_length=MAX_EPISODE_LENGTH, worker_number=0, n_envs=N_EPS, timesteps_per_call=timesteps_per_call
    )
    ...
    for i in range(n_calls):
        ...
        if timesteps_per_call * i < 4:
            assert not any(step_type == StepType.TERMINAL for step_type in eps.step_types)
    worker.shutdown()
...
def test_in_local_sampler(policy, envs, other_envs, timesteps_per_call):
    ...
    vec_workers = WorkerFactory(
        seed=100, n_workers=1, worker_class=FragmentWorker, worker_args=worker_args, max_episode_length=MAX_EPISODE_LENGTH
    )
...
def test_update_envs_env_update(timesteps_per_call):
    ...
    n_workers = 8
    policies = [
        FixedPolicy(env.spec, scripted_actions=[env.action_space.sample() for _ in range(max_episode_length)])
    ...
    workers = WorkerFactory(
        ...
        worker_args=dict(n_envs=1, timesteps_per_call=timesteps_per_call),
    )
