...
def test_update_envs_env_update():
    ...
    episodes = sampler.obtain_samples(0, 161, np.asarray(policy.get_param_values()), env_update=tasks.sample(n_workers))
    mean_rewards = []
    goals = []
...
def test_init_with_env_updates():
    ...
    n_workers = 8
    workers = WorkerFactory(seed=100, max_episode_length=max_episode_length, n_workers=n_workers)
    sampler = LocalSampler.from_worker_factory(workers, policy, envs=tasks.sample(n_workers))
...
def test_no_seed():
    ...
    policy = FixedPolicy(env.spec, scripted_actions=[env.action_space.sample() for _ in range(max_episode_length)])
    n_workers = 8
    workers = WorkerFactory(seed=None, max_episode_length=max_episode_length, n_workers=n_workers)
    sampler = LocalSampler.from_worker_factory(workers, policy, env)
    episodes = sampler.obtain_samples(0, 160, policy)
    assert sum(episodes.lengths) >= 160
...
def test_init_without_worker_factory():
    ...
    env = PointEnv()
    policy = FixedPolicy(env.spec, scripted_actions=[env.action_space.sample() for _ in range(max_episode_length)])
    sampler = LocalSampler(agents=policy, envs=env, seed=100, max_episode_length=max_episode_length)
