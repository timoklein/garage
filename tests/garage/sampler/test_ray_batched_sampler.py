...
def test_update_envs_env_update(ray_local_session_fixture):
    ...
    episodes = sampler.obtain_samples(0, 160, np.asarray(policy.get_param_values()), env_update=tasks.sample(n_workers))
    mean_rewards = []
    goals = []
...
def test_init_with_env_updates(ray_local_session_fixture):
    ...
    n_workers = 8
    workers = WorkerFactory(seed=100, max_episode_length=max_episode_length, n_workers=n_workers)
    sampler = RaySampler.from_worker_factory(workers, policy, envs=tasks.sample(n_workers))
...
def test_init_without_worker_factory(ray_local_session_fixture):
    ...
    env = PointEnv()
    policy = FixedPolicy(env.spec, scripted_actions=[env.action_space.sample() for _ in range(max_episode_length)])
    sampler = RaySampler(agents=policy, envs=env, seed=100, max_episode_length=max_episode_length)
    ...
    assert sampler._worker_factory._max_episode_length == worker_factory._max_episode_length
