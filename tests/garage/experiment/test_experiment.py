...
def _run_launcher(launcher_path, prefix):
    ...
    output = (
        (subprocess.check_output((sys.executable, str(launcher_path)), stderr=subprocess.STDOUT))
        .decode("utf-8")
        .strip()
        .split("\n")
    )
...
def test_wrap_experiment_builds_git_archive():
    ...
    with tempfile.TemporaryDirectory() as launcher_dir:
        ...
        contents = subprocess.check_output(("tar", "--list", "--file", archive_path)).decode("utf-8")
