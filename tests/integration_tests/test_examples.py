...
def test_dqn_pong():
    ...
    assert (
        subprocess.run(
            [EXAMPLES_ROOT_DIR / "tf/dqn_pong.py", "--buffer_size", "5", "--max_episode_length", "5"], check=False, env=env
        ).returncode
        == 0
    )
...
def test_dqn_atari():
    ...
    assert (
        subprocess.run(
            [EXAMPLES_ROOT_DIR / "torch/dqn_atari.py", "Pong", "--buffer_size", "1", "--max_episode_length", "1"],
            check=False,
            env=env,
        ).returncode
        == 0
...
def test_ppo_memorize_digits():
    ...
    env = os.environ.copy()
    env["GARAGE_EXAMPLE_TEST_N_EPOCHS"] = "1"
    command = [EXAMPLES_ROOT_DIR / "tf/ppo_memorize_digits.py", "--batch_size", "4", "--max_episode_length", "5"]
...
def test_trpo_cubecrash():
    ...
    assert (
        subprocess.run(
            [EXAMPLES_ROOT_DIR / "tf/trpo_cubecrash.py", "--batch_size", "4", "--max_episode_length", "5"],
            check=False,
            env=env,
        ).returncode
        == 0
...
def test_step_gym_env():
    """Test step_env.py."""
    assert subprocess.run([EXAMPLES_ROOT_DIR / "step_gym_env.py", "--n_steps", "1"], check=False).returncode == 0
...
def test_step_dm_control_env():
    ...
    assert subprocess.run([EXAMPLES_ROOT_DIR / "step_dm_control_env.py", "--n_steps", "1"], check=False).returncode == 0
...
def test_mtsac_metaworld_mt1_pick_place():
    ...
    env["GARAGE_EXAMPLE_TEST_N_EPOCHS"] = "1"
    assert (
        subprocess.run([EXAMPLES_ROOT_DIR / "torch/mtsac_metaworld_mt1_pick_place.py"], check=False, env=env).returncode == 0
    )
...
def test_mtsac_metaworld_mt10():
    ...
    assert (
        subprocess.run(
            [EXAMPLES_ROOT_DIR / "torch/mtsac_metaworld_mt10.py", "--n_tasks", "10"], check=False, env=env
        ).returncode
        == 0
    )
...
def test_mtsac_metaworld_mt50():
    ...
    assert (
        subprocess.run(
            [EXAMPLES_ROOT_DIR / "torch/mtsac_metaworld_mt50.py", "--n_tasks", "50"], check=False, env=env
        ).returncode
        == 0
    )
...
def test_mtppo_metaworld_mt1_push():
    ...
    assert (
        subprocess.run(
            [EXAMPLES_ROOT_DIR / "torch/mtppo_metaworld_mt1_push.py", "--epochs", "1", "--batch_size", "1"], check=False
        ).returncode
        == 0
    )
...
def test_mttrpo_metaworld_mt1_push():
    ...
    assert (
        subprocess.run(
            [EXAMPLES_ROOT_DIR / "torch/mttrpo_metaworld_mt1_push.py", "--epochs", "1", "--batch_size", "1"], check=False
        ).returncode
        == 0
    )
