    ...
    * An unsuccessful sequence truncated by time limit will look like:
    ...
    def get_step_type(cls, step_cnt, max_episode_length, done):
        ...
        if max_episode_length is not None and step_cnt >= max_episode_length:
            ...
            raise ValueError(
                "Expect step_cnt to be >= 1, but got {} " "instead. Did you forget to call `reset(" ")`?".format(step_cnt)
            )
        else:
            return StepType.MID
...
class TimeStepBatch:
    ...
    def concatenate(cls, *batches):
        ...
        episode_infos = {k: np.concatenate([b.episode_infos[k] for b in batches]) for k in batches[0].episode_infos.keys()}
...
class EpisodeBatch(TimeStepBatch):
    ...
    def __init__(
        ...
        if len(lengths.shape) != 1:
            raise ValueError(f"lengths has shape {lengths.shape} but must be a ternsor of " f"shape (N,)")

        if not (lengths.dtype.kind == "u" or lengths.dtype.kind == "i"):
            raise ValueError(f"lengths has dtype {lengths.dtype}, but must have an " f"integer dtype")
        ...
        for key, val in episode_infos.items():
            if not isinstance(val, np.ndarray):
                raise ValueError(
                    f"Entry {key!r} in episode_infos is of type {type(val)!r} " f"but must be of type {np.ndarray!r}"
                )
        ...
        check_timestep_batch(self, np.ndarray, ignored_fields={"next_observations", "episode_infos"})
    ...
    def from_list(cls, env_spec, paths):
        ...
        if all(len(path["observations"]) == length + 1 for (path, length) in zip(paths, lengths)):
            ...
            if paths[0].get("next_observations") is not None:
                last_observations = np.asarray([p["next_observations"][-1] for p in paths])
            else:
                last_observations = np.asarray([p["observations"][-1] for p in paths])
        ...
        if "dones" in stacked_paths and "step_types" not in stacked_paths:
            step_types = np.array(
                [StepType.TERMINAL if done else StepType.MID for done in stacked_paths["dones"]], dtype=StepType
            )
    ...
    def next_observations(self):
        ...
        """
        return np.concatenate(tuple([np.concatenate((eps.observations[1:], eps.last_observations)) for eps in self.split()]))
    ...
    def episode_infos(self):
        ...
        """
        return {
            key: np.concatenate([np.repeat([v], length, axis=0) for (v, length) in zip(val, self.lengths)])
    ...
    def valids(self):
        ...
        """
        return pad_batch_array(np.ones_like(self.rewards), self.lengths, self.env_spec.max_episode_length)
    ...
    def padded_next_observations(self):
        ...
        """
        return pad_batch_array(self.next_observations, self.lengths, self.env_spec.max_episode_length)
    ...
    def padded_agent_infos(self):
        ...
        """
        return {
            k: pad_batch_array(arr, self.lengths, self.env_spec.max_episode_length) for (k, arr) in self.agent_infos.items()
        }
    ...
    def padded_env_infos(self):
        ...
        """
        return {k: pad_batch_array(arr, self.lengths, self.env_spec.max_episode_length) for (k, arr) in self.env_infos.items()}
...
def check_timestep_batch(batch, array_type, ignored_fields=()):
    ...
    for field, value in fields.items():
        if field in ["observations", "actions", "rewards", "next_observations", "step_types"]:
        ...
        if hasattr(value, "shape"):
            ...
            if env_spec and field in ["observations", "next_observations"]:
                ...
                if isinstance(value[0], np.ndarray) and not env_spec.observation_space.contains(value[0]):
                    warnings.warn(f"Observation {value[0]!r} is outside " f"observation_space {env_spec.observation_space}")
        ...
        if field in ["agent_infos", "env_infos", "episode_infos"]:
            for key, val in value.items():
                if not isinstance(val, (array_type, dict)):
                    raise ValueError(f"Entry {key!r} in {field} is of type {type(val)}" f"but must be {array_type!r} or dict")
                if hasattr(val, "shape"):
        ...
        if (
            ...
            raise ValueError(f"step_types has dtype {value.dtype} but must have " f"dtype StepType")
