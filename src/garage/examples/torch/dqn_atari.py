...
def main(env=None, seed=24, n=psutil.cpu_count(logical=False), buffer_size=None, n_steps=None, max_episode_length=None):
    ...
    if n_steps is not None:
        hyperparams["n_epochs"] = math.ceil(
            int(n_steps) / (hyperparams["steps_per_epoch"] * hyperparams["sampler_batch_size"])
        )
    if buffer_size is not None:
        hyperparams["buffer_size"] = buffer_size

    dqn_atari(dict(log_dir=logdir), env=env, seed=seed, n_workers=n, max_episode_length=max_episode_length, **hyperparams)
