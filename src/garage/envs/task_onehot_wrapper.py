...
class TaskOnehotWrapper(Wrapper):
    ...
    def __init__(self, env, task_index, n_total_tasks):
        ...
        self._observation_space = akro.Box(np.concatenate([env_lb, one_hot_lb]), np.concatenate([env_ub, one_hot_ub]))
        self._spec = EnvSpec(
    ...
    def step(self, action):
        ...
        return EnvStep(
            env_spec=self.spec, action=action, reward=es.reward, observation=oh_obs, env_info=env_info, step_type=es.step_type
        )
    ...
    def wrap_env_cons_list(cls, env_cons):
        ...
        for i, con in enumerate(env_cons):
            ...
            wrapped.append(lambda i=i, con=con: cls(con(), task_index=i, n_total_tasks=n_total_tasks))
        return wrapped
