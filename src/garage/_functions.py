...
def obtain_evaluation_episodes(policy, env, max_episode_length=1000, num_eps=100, deterministic=True):
    ...
    with click.progressbar(range(num_eps), label="Evaluating") as pbar:
        for _ in pbar:
            eps = rollout(env, policy, max_episode_length=max_episode_length, deterministic=deterministic)
            episodes.append(eps)
...
def log_multitask_performance(itr, batch, discount, name_map=None):
    ...
    for task_name in task_names:
        if task_name in eps_by_name:
            ...
            log_performance(itr, EpisodeBatch.concatenate(*episodes), discount, prefix=task_name)
        else:
