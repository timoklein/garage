...
def _make_experiment_signature(function):
    ...
    for param in func_sig.parameters.values():
        if not saw_first_param:
            ...
            if param.name != "ctxt":
                raise ValueError(
                    "Experiment functions should have a first " "parameter named 'ctxt' instead of {!r}".format(param.name)
                )
            saw_first_param = True
        else:
            new_params.append(
                inspect.Parameter(
                    name=param.name, kind=inspect.Parameter.KEYWORD_ONLY, default=param.default, annotation=param.annotation
                )
            )
    if not saw_first_param:
        raise ValueError(
            "Experiment functions should have a first parameter " "named 'ctxt', but {!r} has no parameters".format(function)
        )
    ...
    def _make_context(cls, options, **kwargs):
        ...
        if log_dir is None:
            log_dir = "{data}/local/{prefix}/{name}".format(
                data=os.path.join(os.getcwd(), "data"), prefix=options["prefix"], name=name
            )
        ...
        return ExperimentContext(
            snapshot_dir=log_dir, snapshot_mode=options["snapshot_mode"], snapshot_gap=options["snapshot_gap"]
        )
...
def get_metadata():
    ...
    try:
        git_root_path = subprocess.check_output(
            ("git", "rev-parse", "--show-toplevel"), cwd=os.path.dirname(main_file_path), stderr=subprocess.DEVNULL
        )
    ...
    launcher_path = os.path.relpath(bytes(main_file_path, encoding="utf8"), git_root_path)
    git_hash = subprocess.check_output(("git", "rev-parse", "HEAD"), cwd=git_root_path)
    git_hash = git_hash.decode("utf-8").strip()
    git_status = subprocess.check_output(("git", "status", "--short"), cwd=git_root_path)
...
def make_launcher_archive(*, git_root_path, log_dir):
    ...
    git_files = subprocess.check_output(
        ("git", "ls-files", "--others", "--exclude-standard", "--cached", "-z"), cwd=git_root_path
    ).strip()
    repo_size = 0
    files_to_archive = []
    ...
    subprocess.run(
        ("tar", "--null", "--files-from", "-", "--xz", "--create", "--file", archive_path),
...
class LogEncoder(json.JSONEncoder):
    ...
    # Modules whose contents cannot be meaningfully or safely jsonified.
    BLOCKED_MODULES = {
        "ray",
        "itertools",
    }
