...
def td3_garage_pytorch(ctxt, env_id, seed):
    """Create garage Pytorch TD3 model and training.
    ...
    num_timesteps = hyper_parameters["n_epochs"] * hyper_parameters["steps_per_epoch"] * hyper_parameters["batch_size"]
    ...
    qf1 = ContinuousMLPQFunction(
        env_spec=env.spec, hidden_sizes=hyper_parameters["qf_hidden_sizes"], hidden_nonlinearity=F.relu
    )
    ...
    replay_buffer = PathBuffer(capacity_in_transitions=hyper_parameters["replay_buffer_size"])
    ...
    trainer.train(n_epochs=hyper_parameters["n_epochs"], batch_size=hyper_parameters["batch_size"])
